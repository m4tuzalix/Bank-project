...
class bank(tk.Tk):
    ...
    # Submenus
        ...
        Sub3.add_command(label="Open admin chat", command=lambda: admin_chat(self))
        ...
        if not self.secur:
            ...
            if self.amt < 0:
                ...
                with open(self.path_customers+self.name, "w") as file:
                    ...
                    file.write(lines[1])
                    file.write(lines[2])
                    file.write("%s \n" % (float(lines[3])+self.amt))                ## If conditions are made it opens the file and overwrites the line responsibile for ballance
                    file.write(lines[4])
                    file.write(lines[5])
                    file.write(lines[6])
                    file.write(lines[7])
